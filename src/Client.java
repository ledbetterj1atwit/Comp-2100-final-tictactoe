	...
	public static void main(String[] args) throws IOException {
		...
		if ((serverResponse.equals(yes)) && (response.contentEquals(yes))) {
			...
			while (playing) { // game loop
				...
				init(board, oWins, oWins);
				display(board);
				while (true) { // round loop
					System.out.printf("X's Turn%n");
					serverMove = dInput.readUTF(); // Wait for server.
					if (serverMove.contentEquals(end)) { // Check for quit.
						System.out.printf("%nOther player quit!%n");
						playing = false;
						break;
					}
					updateBoard(Integer.parseInt(serverMove), 'X', board); // Update
					updateWins(Integer.parseInt(serverMove), oWins);
					if (checkWin(board, xWins, 'X')) { // check for server win
						...
						if (!bF1.readLine().contentEquals(yes)) {
							...
							serverResponse = dInput.readUTF();
							if (serverResponse.contentEquals(end))
								playing = false;
							break;
						}
					} else if (checkTie(oWins, xWins)) { // check for tie
					...
					updateBoard(Integer.parseInt(clientMove), 'O', board); // update
					updateWins(Integer.parseInt(clientMove), xWins);
					if (checkWin(board, oWins, 'O')) { // check client win
					...
					} else if (checkTie(oWins, xWins)) { // check tie
						System.out.println("Its a tie! Do you wish to play again?(Y/N)");
						if (!bF1.readLine().contentEquals(yes)) {
							playing = false;
							dOutput.writeUTF(end);
